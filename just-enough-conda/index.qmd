...
I will focus mostly on the `conda`, but you may have hard of `pip`, `uv`, `mamba` / `micromamba`, or `pixi`.[^tools]
The principles discussed here generally apply to any major Python package manager.

[^tools]: Mentally I divide these tools into two groups: Python-only or multi-language.
...
- You want to prevent software upgrades from breaking your code, so you want to specify _which_ version of a package you need for a project.
...
...the terminal _inside of VScode_ will have inherited the environment configuations.
...
- _The website env does not / should not care_ what is in each blog post env, and vice versa, so it makes sense for these envs to be separate!
...
It controls _your machine only_.
